        ...
        <table>
            @foreach ($equipe->componentes as $index => $item)
                ...
                <td>
                    <p style="font-size: 14px;"><b>{{$item['nome']}} ({{$item['funcao']}})</b></p>
                    <p style="font-size: 14px;">{{$item['endereco']}}</p>
                    <p style="font-size: 13px;"><b>DN</b>: {{$item['nacimento']}} <b>Fone:</b> {{$item['telefone']}} </p><br>
                </td>
            @endforeach
        </tr>
        </table>
    @else
    Preencha o formulário inserindo <br>os compeonentes jovens da equipe
    @endif
    </body>
</html>